...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def upcoming(self, request):
        ...
        upcoming_bookings = self.get_queryset().filter(
            booking_date__gte=today, status__in=["PENDING", "CONFIRMED"]
        )
    ...
    def past(self, request):
        ...
        """
        today = timezone.now().date()
        past_bookings = self.get_queryset().filter(booking_date__lt=today)
...
class BookingAvailabilityView(APIView):
    ...
    def get(self, request):
        ...
        if not resource_id or not date_str:
            return Response(
                {"error": "resource_id and date are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        bookings = Booking.objects.filter(
            resource=resource, booking_date=date, status__in=["PENDING", "CONFIRMED"]
        ).order_by("start_time")
